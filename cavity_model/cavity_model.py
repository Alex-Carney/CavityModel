...
class CavityModel:
    ...
    def plot_steady_state(self, complex_photon_response: np.ndarray) -> None:
        """
        ...
        plt.title("Cavity Occupancy by Driving Frequency")
        plt.show()
...
if __name__ == '__main__':
    ...
    simple_adj = np.array([[0, 1], [1, 0]])
    # Do not delete: Example from fitting to Juan's data to pick out gamma and J
    #wf_sweep = np.r_[4.36:4.5:.001]
    #cavity_freqs = np.array([[4.42], [4.42]])
    # J : .02 // Gamma : .001
    wf_sweep = np.r_[6.5:7.5:.001]
    cavity_freqs = np.array([[6.995], [6.996]])
    single_cavity_drive = np.array([[1], [0]])
    gamma_experimentally_fitted = np.array([[.001], [.001]])
    ...
    # Whatever works
    SCALE_FACTOR = 9e-6
    #cm.plot_steady_state(complex_response * SCALE_FACTOR)
